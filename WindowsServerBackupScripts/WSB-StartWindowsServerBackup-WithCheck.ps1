...
#region MAIN - BACKUP CHECK
function Start-BackupTheBackups {
    # Start the log file
    New-TaskLogFile
...
#region START BACKUP
function Start-ScheduledServerBackup() {
    $policy = New-WBPolicy
    $skipVMsNamed = "*test*", "*no*back*up*", "*(dbu)*"
    $VMs = Get-WBVirtualMachine | Where-Object {$_.VMName -notlike $skipVMsNamed}
...
function New-TaskLogFile {
    ...
    $logFileName = "Start-BackuptheBackupsLog_$($client)_$($hostname)_$($logDate).txt"

...
function Get-LastBackupSuccess {
    ...
    $LastWSBSuccessEvent = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Backup'; StartTime=$checkDate; Id='4'} -ErrorAction Ignore
...
function Get-TotalSizeofVHDs {
    ...
    foreach ($vm in $backupVMs) {
        ...
        if (Get-VMSnapshot -VMName $vmName) {
            foreach ($d in $disks) {
                $diskParent = $d.Path | Split-Path -Parent
                $allVMdisks = Get-ChildItem -Path $diskParent -Recurse | Where-Object {$_.Name -like "$vmName*.*vhd*"}
                foreach ($d in $allVMdisks) {$backupVHDs += $d.FullName}
            }  
        } else {foreach ($d in $disks) {$backupVHDs += $d.Path}}
    }
    $vhdSizes = foreach ($disk in $backupVHDs) {$(Get-Item -Path $disk).Length}

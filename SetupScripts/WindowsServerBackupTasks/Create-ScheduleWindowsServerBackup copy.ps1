...
# This script also creates or updates a task to run WSB-BackuptheBackups.ps1. This
# should run 30 minutes before the scheduled backup, and after the Friday backup
# unless the server also runs backups on weekends.

# TASK VARIABLES
$startBackupTaskProperties = @{
    taskName = "MITKY - Schedule Windows Server Backup"
    updateTaskName = "MITKY - Schedule Windows Server Backup"
...
$rotateBackupsTaskProperties = @{
    taskName = "MITKY - Backup the Backups"
    updateTaskName = "MITKY - Backup the Backups"
...
If ($scheduledBackup) {$startBackupTaskProperties.backupStartTime = Get-Date $((Get-WBSummary).LastBackupTime) -Format "HH:mm"}
Elseif (Get-ScheduledTask $startBackupTaskProperties.taskName) {$startBackupTaskProperties.backupStartTime = Get-Date $((Get-ScheduledTask $($startBackupTaskProperties.taskName)).Triggers.StartBoundary) -Format "HH:mm"}
$rotateBackupsTaskProperties.startTime = $(Get-Date $startBackupTaskProperties.backupStartTime).AddMinutes(-30) | Get-Date -Format "HH:mm"
...
If ($weekendBackup) {
    $startBackupTaskProperties.taskTrigger = New-ScheduledTaskTrigger -Daily -At $startBackupTaskProperties.backupStartTime
    $rotateBackupsTaskProperties.taskTrigger = New-ScheduledTaskTrigger -Daily -At $rotateBackupsTaskProperties.startTime    
} 
Else {
    $startBackupTaskProperties.taskTrigger = New-ScheduledTaskTrigger -Weekly -At $startBackupTaskProperties.backupStartTime -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday
    $rotateBackupsTaskProperties.taskTrigger = New-ScheduledTaskTrigger -Weekly -At $rotateBackupsTaskProperties.startTime -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday
